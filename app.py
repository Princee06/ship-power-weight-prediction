...
def load_models():
    try:
        power_model = joblib.load(MODEL_DIR / "power_pipeline.pkl")
        weight_model = joblib.load(MODEL_DIR / "weight_pipeline.pkl")
        return power_model, weight_model
    except FileNotFoundError:
        st.error("Model files not found! Ensure 'saved_models/' folder is in the repo with .pkl files.")
        return None, None
...
def run_sanity_checks(l, b, d, dr, s, t):
    errors, warnings = [], []
    if dr > d: errors.append("Draft cannot exceed Depth.")
    if l > 450: errors.append("LOA exceeds typical ship limits (450m).")
    if t == "Tug" and l > 100: errors.append("Tugs rarely exceed 100m. Check LOA.")
    if t == "Bulk Carrier" and s > 18: warnings.append("High speed for Bulk Carrier (<16kts typical).")
    lb_ratio = l / b if b else 0
    if lb_ratio != 0 and (lb_ratio < 3.0 or lb_ratio > 12):
        warnings.append(f"Unusual L/B Ratio ({lb_ratio:.2f})")
...
with st.sidebar:
    ...
    with st.expander("🛠 Assumptions & Limitations", expanded=True):
        st.markdown("""
        **Reliability Zone:** LOA 25-400m | Speed 10-25kts  
        **Assumptions:** Standard Steel Hull, Installed Power = Total MCR  
        **Limitations:** No environmental factors, ±8-12% accuracy
        """)
...
selected_type = st.selectbox("Select Ship Type (loads presets)", ["Select...", "OSV", "Tug", "Bulk Carrier", "Container"])
...
with col2:
    ...
    manual_p = 0.0
    if validation_mode:
        manual_p = st.number_input("Manual Power Estimate (kW)", value=0.0)

if st.button("Calculate Results") and power_model and weight_model:
    errs, warns = run_sanity_checks(loa_m, breadth_m, depth_m, draft_m, speed, selected_type)
    if selected_type == "Select...": st.error("Select a Ship Type.")
    elif errs: [st.error(e) for e in errs]
    else:
        [st.warning(w) for w in warns]
        ...
        try:
            ...
            st.info("SHAP simplified for this view.")
...
if up_file and power_model and weight_model:
    ...
    if all(c in b_df.columns for c in essential):
        ...
        if st.checkbox("Global SHAP Trends"):
        ...
        st.error(f"Missing columns: {essential}")
