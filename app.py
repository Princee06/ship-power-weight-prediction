import streamlit as st
import pandas as pd
import joblib
import matplotlib.pyplot as plt

# ---- Optional SHAP Import ---- #
try:
    import shap
    SHAP_AVAILABLE = True
except:
    SHAP_AVAILABLE = False

# ---- 0. Page Config ---- #
st.set_page_config(page_title="Ship Power & Weight Prediction", layout="wide")

# ---- 1. Load Models ---- #
@st.cache_resource
def load_models():
    p_model = joblib.load("saved_models/power_pipeline.pkl")
    w_model = joblib.load("saved_models/weight_pipeline.pkl")
    return p_model, w_model

power_model, weight_model = load_models()

# ---- 2. Helper Logic ---- #
PRESETS = {
    "Tug": {"loa": 32.0, "breadth": 11.0, "depth": 5.0, "draft": 4.0, "speed": 12.0},
    "Container": {"loa": 300.0, "breadth": 48.0, "depth": 24.0, "draft": 13.0, "speed": 22.0},
    "Bulk Carrier": {"loa": 225.0, "breadth": 32.2, "depth": 19.0, "draft": 14.2, "speed": 14.5},
    "OSV": {"loa": 85.0, "breadth": 19.0, "depth": 8.0, "draft": 6.5, "speed": 13.0}
}

def run_sanity_checks(l, b, d, dr, s, t):
    errors = []
    warnings = []
    if dr > d: errors.append("Draft cannot be deeper than Depth.")
    if l > 450: errors.append("LOA exceeds maximum known ship limits (450m).")
    if t == "Tug" and l > 100: errors.append("Standard Tugs rarely exceed 100m.")
    if t == "Bulk Carrier" and s > 18: warnings.append("Speed is high for Bulk Carrier.")
    lb_ratio = l/b if b > 0 else 0
    if lb_ratio != 0 and (lb_ratio < 3.0 or lb_ratio > 12):
        warnings.append(f"Unusual L/B Ratio ({lb_ratio:.2f}).")
    return errors, warnings

# ---- 3. Feature Engineering ---- #
def engineer_features(df):
    df["L_B"] = df["loa_m"] / df["breadth_m"]
    df["B_D"] = df["breadth_m"] / df["depth_m"]
    df["L_D"] = df["loa_m"] / df["depth_m"]
    df["speed_length_ratio"] = df["service_speed_kn"] / (df["loa_m"] ** 0.5)
    df["year_bucket"] = pd.cut(
        df["year_built"],
        bins=[0, 1999, 2009, 2019, 2035],
        labels=["old", "mid", "modern", "latest"]
    )

    default_cols = {
        "block_coefficient": 0.7, "fresh_water_capacity_m3": 100,
        "dp_class": "DP1", "deck_area_m2": 500, "dataset_source": "manual",
        "number_of_engines": 2, "bollard_pull_t": 50, "deck_load_t": 200,
        "strata": "medium", "number_of_thrusters": 2, "payload_t": 1000,
        "level1_group": "support", "cargo_pump_capacity_m3hr": 0,
        "engine_type": "diesel", "dwt_t": 5000, "size_band": "medium",
        "teu_capacity": 0, "displacement_t": 8000,
        "propeller_diameter_m": 4, "lpp_m": df["loa_m"] * 0.95,
        "propulsion_type": "conventional", "fuel_capacity_m3": 300
    }

    for col, val in default_cols.items():
        if col not in df.columns:
            df[col] = val

    return df

# ---- Sidebar ---- #
with st.sidebar:
    st.header("Engineering Panel")
    st.info("AI Model v2.0")

# ---- Main UI ---- #
st.title("Ship Power & Weight Prediction")

selected_type = st.selectbox(
    "Select Ship Type",
    ["Select...", "OSV", "Tug", "Bulk Carrier", "Container"]
)

defaults = PRESETS.get(selected_type, {"loa":0,"breadth":0,"depth":0,"draft":0,"speed":0})

col1, col2 = st.columns(2)

with col1:
    loa_m = st.number_input("LOA", value=defaults["loa"])
    breadth_m = st.number_input("Breadth", value=defaults["breadth"])
    depth_m = st.number_input("Depth", value=defaults["depth"])
    draft_m = st.number_input("Draft", value=defaults["draft"])

with col2:
    speed = st.number_input("Speed", value=defaults["speed"])
    year_built = st.number_input("Year", value=2024)

# ---- Prediction ---- #
if st.button("Predict"):
    errs, warns = run_sanity_checks(loa_m, breadth_m, depth_m, draft_m, speed, selected_type)

    if selected_type == "Select...":
        st.error("Select ship type")
    elif errs:
        for e in errs: st.error(e)
    else:
        for w in warns: st.warning(w)

        df = pd.DataFrame([{
            "loa_m": loa_m,
            "breadth_m": breadth_m,
            "depth_m": depth_m,
            "draft_m": draft_m,
            "service_speed_kn": speed,
            "year_built": year_built,
            "ship_type": selected_type
        }])

        df = engineer_features(df)

        power = power_model.predict(df)[0]
        weight = weight_model.predict(df)[0]

        st.success(f"Power: {power:.2f} kW")
        st.success(f"Weight: {weight:.2f} tons")

        # ---- SHAP Section ---- #
        st.markdown("---")
        st.subheader("Feature Contribution")

        if SHAP_AVAILABLE:
            try:
                num_data = power_model[:-1].transform(df)
                explainer = shap.TreeExplainer(power_model.named_steps["model"])
                shap_values = explainer(num_data)

                fig, ax = plt.subplots()
                shap.plots.bar(shap_values[0], show=False)
                st.pyplot(fig)
            except:
                st.info("SHAP failed for this input.")
        else:
            st.info("SHAP not available in deployment.")

# ---- Batch ---- #
st.markdown("---")
file = st.file_uploader("Upload CSV")

if file:
    df = pd.read_csv(file)
    df = engineer_features(df)

    df["power"] = power_model.predict(df)
    df["weight"] = weight_model.predict(df)

    st.dataframe(df.head())
    st.download_button("Download", df.to_csv(index=False), "results.csv")
